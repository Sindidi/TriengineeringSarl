...
public class ProduitAutoPanel extends JPanel {
	/**
	 * 
	 */
	...
	public ProduitAutoPanel() {
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql1 = "SELECT  * FROM `automobile` WHERE marque = '"+search+"' "
						+ "or  ref1 = '"+search+"' or  ref2 = '"+search+"' or  type = '"+search+"' or annee = '"+search+"'";
				try {
		...
		btnSupp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String sql = "DELETE FROM `automobile` WHERE Id = '"+id+"'";
				try {
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblAvatar.setIcon(null);
				String sql = "select * from automobile where Id='"+id+"'";
				try {
					pst = con.prepareStatement(sql);

					rs = pst.executeQuery();
					if(rs.next()) {
						modifierAuto.tfQty.setText(rs.getString("qt"));
						modifierAuto.formattedtfAnnee.setText(rs.getString("annee"));
						modifierAuto.comboBoxType.setSelectedItem(rs.getString("type"));
						modifierAuto.tfRef1.setText(rs.getString("ref1"));
						modifierAuto.tfRef2.setText(rs.getString("ref2"));
						modifierAuto.tfMarque.setText(rs.getString("marque"));


						byte[] img = rs.getBytes("photo");
						ImageIcon image = new ImageIcon(img);
						java.awt.Image im = image.getImage();
						java.awt.Image myImg = im.getScaledInstance(modifierAuto.lblPhoto.getWidth(), modifierAuto.lblPhoto.getHeight(), java.awt.Image.SCALE_SMOOTH);
	...
	public void updateTable() {
		...
		String sql = "select id, qt,annee,type,ref1,ref2,marque from automobile";
		try {
			pst = con.prepareStatement(sql);
			rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
