...
public class ModifierElect extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModifierElect frame = new ModifierElect();
	...
	public ModifierElect() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(src == null) {
					...
					if(!qty.equals("") &&  !name.equals("") && !marque.equals("") && !ref.equals("") && !desc.equals("")) {
						try {
							String sql = "UPDATE `elecrique` SET `qt`=?, `nom`=?, `marque`=?, `ref`=?, `description`=?,`fiche`=? where Id='"+key+"'";
							pst = con.prepareStatement(sql); 
							pst.setString(1, tfQty.getText().toString());
							pst.setString(2, tfName.getText().toString());
							pst.setString(3, tfMarque.getText().toString());
							pst.setString(4, tfRef.getText().toString());
							pst.setString(5, textArea.getText().toString());
							pst.setString(6, tfFicheTech.getText().toString());
							pst.execute();
					...
					if(!qty.equals("") &&  !name.equals("") && !marque.equals("") && !ref.equals("") && !desc.equals("")) {
						try {
							...
							String sql = "UPDATE `elecrique` SET `qt`=?, `nom`=?, `marque`=?, `ref`=?, `description`=?,`fiche`=?, `photo`=? where Id='"+key+"'";
							pst = con.prepareStatement(sql); 
							pst.setString(1, tfQty.getText().toString());
							pst.setString(2, tfName.getText().toString());
							pst.setString(3, tfMarque.getText().toString());
		...
		lblNewLabel = new JLabel("Modifier un Produit Electrique");
		...
		textArea.setBounds(10, 205, 238, 77);
